                    ...
                    const int max_iter)
{
	...
	imagesection[i][j] = (*in_set_fn)(x0, y0, max_iter);
        }
    }
    return imagesection;
}
...
void writeConfig(const int *nProcs, const int * gridX, const int * gridY, const int *iter,
                 const float * xMin, const float * xMax, const float * yMin, const float * yMax, const int *pixels_in_task){
                 ...
                 MPI_Comm comm)
{
    ...
    writeConfig(&size, &grid_size_x, &grid_size_y, &max_iter,  &xmin, &xmax, &ymin, &ymax, &pixels_in_task);
    startTime = getTime();
    /* End Gathering */
...
int main(int argc, char** argv)
{
    ...
    read_options(argc, argv, &grid_size_x, &grid_size_y, &max_iter,
                 &xmin, &xmax, &ymin, &ymax, &function_name,&pixels_in_task,
                 &shading);
